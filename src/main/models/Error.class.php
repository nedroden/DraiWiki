...
class Error extends ModelController {
	...
	public function __construct($detailedInfo, $parameters, $langFallBack = false) {
		...
		$this->_title = !$langFallBack ? $this->locale->read('error', 'an_error_occurred') : 'Fatal error';
		$this->_body = !$langFallBack ? $this->locale->read('error', 'an_error_occurred_message') : 'A fatal error has just occured, and for security reasons, the script has been aborted. We apologize for any inconvenience this is causing. Try refreshing the page to see if this error has been resolved. If not, please contact the administrator.';
